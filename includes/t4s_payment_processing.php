...
function grab_T4Sform($atts){
	...
	if ($_POST['x_login'] != '') {
		...
		mysqli_query($mysqli, $query) or die("Internal error. Please contact us for assistance.");
		...
		<form method='post' id='t4s_form' action="https://secure.authorize.net/gateway/transact.dll">
			...
			<input type="hidden" name="x_relay_url" value="<?php echo admin_url( 'admin-ajax.php', 'https' ).'?action=t4s_auth_ipn_callback'; ?>" >
	...
	if ($_POST['cmd'] == '_xclick') {
		...
		mysqli_query($mysqli, $query) or die("Internal error. Please contact us for further assistance.");
		...
		?>
			...
			<form name="t4s_form" action="https://www.paypal.com/cgi-bin/webscr" method="post">
				...
				<input type="hidden" name="notify_url" value="<?php echo admin_url( 'admin-ajax.php', 'https' ).'?action=t4s_ppl_ipn_callback'; ?>">
	...
	if ($row2['pmt_type'] == 'fixed') {
		$preview .= "<tr><td><strong>Total Due: $".money_format('%i', $price)."</strong></td></tr>";
	} else if ($row2['pmt_type'] == 'user') {
		$preview .= "<tr><td>Please specify how much you would like to pay: </td><td><b>$</b><input type='text' name='price' id='price'  value='0' size=6 onChange='checkNumeric(this.id)'>.<input type='text' name='price2' id='price2'  value='00' size=2 onChange='checkNumeric(this.id)'></td></tr> 
		";
	} else {
		$preview .= "<tr><td><strong>No payment is necessary</strong><br/><br/><input type='submit' value='Finish'></td></tr>";
		$nopmt = true;
	}
	...
	if ($gateway == 2 && !$nopmt) {
		...
		mysqli_query($mysqli, $query) or die("An error had occured. Please contact us for further assistance.");
		
		$invoice = mysqli_insert_id();
		if ($api_login_id == '') {
			echo "Payment gateway must be specified and credentials entered before this form could be displayed.";
		} else {
		
		?>
		<form method='post' class='t4sCustomForm'>
			...
			<input type='hidden' name='x_invoice_num' value='<?php echo $invoice; ?>'>
			<input type="hidden" name="x_relay_url" value="<?php admin_url( 'admin-ajax.php', 'https' ).'?action=t4s_auth_ipn_callback'; ?>" >
			<input type='submit' value="Continue" style="border: 0">
	...
	<?php
		...
		<form name="_xclick" method="post" class='t4sCustomForm'>
